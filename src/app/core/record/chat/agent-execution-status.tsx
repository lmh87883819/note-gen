...
function extractToolContent(call: any): string {
  ...
  const content =
    data?.data?.content ?? data?.data?.tool_result?.results?.[0]?.content ?? "";
...
export function AgentExecutionStatus() {
  ...
  const statusTitle = agentState.pendingConfirmation
              ...
              <div key={call.id} className="space-y-1">
                ...
                {expanded && (
                  <div className="pl-6 pr-3 pb-2 text-xs text-muted-foreground">
                    {isDiffLike(call) && (
                      <div className="mt-2">
                        <div className="font-medium text-xs mb-1 text-foreground/80">
                          Diff
                        </div>
                        <div className="agent-diff-box">
                          {(extractToolContent(call) || "(no diff)")
                            .split("\n")
                            .map((line, idx) => (
